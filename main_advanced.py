...
┌─────────────────────────────────────────────────────────────┐
│ 阶段2：少样本学习（提示学习与微调）                              │
│ - 使用少量标注数据（10%, 30%, 50%, 100%）                     │
│ - 冻结预训练编码器，只训练 Prompt + 分类头                     │
│ - 轻量级微调，参数效率高                                       │
└─────────────────────────────────────────────────────────────┘
              ↓ 得到分类模型
┌─────────────────────────────────────────────────────────────┐
...
class AdvancedExperiment:
    ...
    def step1_pretrain(self):
        """
        阶段1：无监督学习（自监督预训练）
        在 SC-2 全量数据上进行 AutoEncoder 预训练
        目标：学习通用的特征表示，无需标签
        """
        print("\n" + "="*60)
    ...
    def step3_evaluation(self, adata_processed):
        ...
        阶段3：评估与解释（多维度分析）
        对所有模型进行全面评估
        指标：ARI/NMI/F1/AUC + 聚类质量 + 特征稳定性 + 计算效率
        """
        print("\n" + "="*60)
        print("步骤3: 多维度评估")
        print("="*60)
        ...
        for ratio_name, result in self.prompt_results.items():
            ...
            with torch.no_grad():
                ...
                y_prob = torch.softmax(logits, dim=1).cpu().numpy()  # 添加概率输出
            ...
            classification_results = self.evaluator.evaluate_classification(
                y_all, y_pred, y_prob=y_prob  # 传入概率以计算 AUC
            )
            ...
            if classification_results.get('auc') is not None:
    ...
    def step4_visualization(self):
        ...
        for ratio in ratios:
            ...
            # 添加 AUC
            auc_value = results['classification'].get('auc', 0.0)
            metrics['auc'].append(auc_value if auc_value is not None else 0.0)
    ...
    def _print_final_summary(self):
        ...
        header5 = "ARI"
        header6 = "NMI"
        print(f"{header1:<15} {header2:<10} {header3:<10} {header4:<10} {header5:<10} {header6:<10}")
        print("-"*70)
        ...
        for ratio_name, results in self.evaluation_results.items():
            ...
            auc_val = results['classification'].get('auc', 0.0)
            auc_val = auc_val if auc_val is not None else 0.0
            ari = results['clustering']['adjusted_rand_index']
            nmi = results['clustering']['normalized_mutual_info']
            print(f"{ratio_display:<15} {acc:<10.4f} {f1:<10.4f} {auc_val:<10.4f} {ari:<10.4f} {nmi:<10.4f}")
