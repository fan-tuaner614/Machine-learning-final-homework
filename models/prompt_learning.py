...
class PromptLearningModel(nn.Module):
    """
    提示学习模型 (Prompt Learning Model)
    端到端流程 - 阶段2：少样本提示学习与微调
    
    架构：
    1. 冻结预训练编码器：保持阶段1的特征提取能力
    2. 可学习提示向量 (Prompt Embeddings): 轻量级可训练参数
    3. 轻量级分类头 (Classifier): 用于少样本任务
    
    优势：
    - 只需微调少量参数（prompt + 分类头）
    - 保持预训练特征的通用性
    - 适用于 10%, 30%, 50%, 100% 标签比例的少样本场景
    """
...
def train_prompt_model(model, train_loader, val_loader, optimizer, device, 
    ...
    训练策略：
    1. 冻结预训练编码器参数（保持不变）
    2. 只更新 prompt embeddings 和分类头参数
    3. 使用交叉熵损失进行监督学习
    4. 早停机制防止过拟合
    
    Args:
        model: PromptLearningModel实例
        train_loader: 训练数据加载器（少量标注样本）
        val_loader: 验证数据加载器
        optimizer: 优化器
        device: 设备
        epochs: 训练轮数
