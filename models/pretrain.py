...
class AutoEncoder(nn.Module):
    ...
    自编码器模型 (AutoEncoder)
    架构：
    1. 编码器 (Encoder): 输入数据 -> 隐藏层 -> 潜在空间 (降维)
    2. 潜在空间 (Latent Space): 低维的特征表示
    3. 解码器 (Decoder): 潜在空间 -> 隐藏层 -> 重构数据 (升维)
    4. 损失函数: MSE (Mean Squared Error) 计算重构误差
    5. 用途：编码器用于特征提取，为下游任务提供预训练特征
    """
...
class SimCLR(nn.Module):
    ...
    SimCLR对比学习模型
    架构：
    1. 共享编码器 (Shared Encoder): 将两个增强视图映射到潜在空间
    2. 投影头 (Projection Head): 将编码特征映射到对比学习空间
    3. 使用 InfoNCE/NT-Xent 损失进行对比学习
    4. 下游任务使用编码器提取的特征（不使用投影头）
    """
...
class ContrastiveLoss(nn.Module):
    """
    NT-Xent (Normalized Temperature-scaled Cross Entropy) 对比损失函数
    也称为 InfoNCE (Information Noise Contrastive Estimation) 损失
    用于 SimCLR 训练
...
class SingleCellDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            # SimCLR 数据增强：生成两个不同的增强视图
            # 视图1：添加高斯噪声 + 随机Dropout/Mask
            x1 = x + torch.randn_like(x) * 0.1  # 高斯噪声增强
            mask1 = torch.rand_like(x) > 0.1     # 随机掩码（90%保留）
            x1 = x1 * mask1
            
            # 视图2：添加高斯噪声 + 随机Dropout/Mask（独立随机）
            x2 = x + torch.randn_like(x) * 0.1  # 高斯噪声增强
            mask2 = torch.rand_like(x) > 0.1     # 随机掩码（90%保留）
            x2 = x2 * mask2
            
            return x1, x2  # 返回两个增强视图用于对比学习
        else:
            return x
...
def train_autoencoder(model, dataloader, optimizer, device, epochs=100):
    ...
    2. 编码器降维: x -> z (潜在空间)
    3. 解码器重构: z -> x_recon
    4. 计算 MSE 损失: loss = MSE(x_recon, x)
    5. 反向传播更新参数
...
def train_simclr(model, dataloader, optimizer, device, epochs=100, temperature=0.5):
    ...
    训练流程：
    1. 对每个 batch，获取两个增强视图 (x1, x2)
    2. 通过共享编码器和投影头得到 (z1, z2)
    3. 计算 InfoNCE/NT-Xent 对比损失
    4. 反向传播更新参数
    
    Args:
        model: SimCLR 模型
        dataloader: 数据加载器（带数据增强）
        optimizer: 优化器
        device: 设备
        epochs: 训练轮数
